...
备份主体由
原作者:cnwangjihe    https://github.com/cnwangjihe
优化作者:yellowstone    https://github.com/CberYellowstone
完成

框架全部由苹果香蕉泥制作    https://github.com/applenana
优化：来蹭饭的希冀[更改了一些反人类设计]    
...
for content in txtlist:
    ...
    if "\n" not in content:
        if "/p/" in content:
            ...
            if "?" in content:
                #print("?存在")
                if content.split("?")[0].split("/p/")[1] not in str(pids):
                    pids.append(int(content.split("?")[0].split("/p/")[1]))
            elif "？" in content:
                if content.split("？")[0].split("/p/")[1] not in str(pids):
                    pids.append(int(content.split("？")[0].split("/p/")[1]))            
            else:
                #print("?不存在")
                if content.split("/p/")[1] not in str(pids):
                    pids.append(int(content.split("/p/")[1]))
    ...
    try:
        ...
        if (pid==0):
            if (PreSet):
                ...
                Avalon.info("已经完成备份！现在进行文件整理，交由file_manager处理")
                f=open("pids.tmp","w",encoding=("utf-8"))
                f.write(str(pids))
                f.close()
